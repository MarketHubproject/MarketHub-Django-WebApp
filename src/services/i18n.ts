...
class SimpleEventEmitter {
  ...
  removeListener(
    event: string,
    listenerToRemove: (...args: any[]) => void
  ): void {
    const eventListeners = this.listeners.get(event);
    if (eventListeners) {
...
class I18nService extends SimpleEventEmitter implements I18nServiceInterface {
  ...
  private async loadFallbackTranslations(
    ...
    try {
      ...
      logger.error(
        `Failed to load fallback translations for language '${language}'`,
        error,
        {
          component: "I18nService",
  ...
  /**
   * Static translation registry to avoid dynamic require issues in React Native
   */
  private getTranslationRegistry(): Record<string, any> {
    return {
  ...
  private async dynamicImport(
    ...
    try {
      const registry = this.getTranslationRegistry();
      const translations = registry[language];

      if (!translations) {
        throw new Error(
          `Translation file for language '${language}' not found`
        );
      }
      ...
      if (language === "en") {
        // Fallback to English if it exists in registry
        const registry = this.getTranslationRegistry();
        const englishTranslations = registry["en"];
  ...
  private keyExistsInTranslations(key: TranslationKey): boolean {
    return (
      this.getTranslationValue(key, this.currentTranslations) !== null ||
  ...
  private logMissingKey(key: TranslationKey): void {
    ...
    if (shouldLog) {
      ...
      if (count >= this.MAX_LOGS_PER_KEY) {
        ...
        logger.warn(
          `${message} (Final warning - will not log again)`,
          null,
          context
        );
      } else {
...
// Create singleton instance - explicitly start with English
const i18n = new I18nService("en", "en");
